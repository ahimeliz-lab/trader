...
const intervalMin = Number(process.env.NEWS_CRON_MIN || 10);
const runDaemon = String(process.env.NEWS_CRON_DAEMON || "false").toLowerCase() === "true";
...
(async function main() {
  ...
  console.log(`news cron running every ${intervalMin}m`);
  await runOnce();
  setInterval(runOnce, Math.max(1, intervalMin) * 60 * 1000);
})().catch((e) => {
  console.error("news cron failed:", e?.message || e);
  process.exit(1);
});

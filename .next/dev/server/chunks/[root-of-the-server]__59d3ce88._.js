...
async function GET(req) {
    try {
        ...
        const sb = sbAdmin();
        const { data: acct, error: acctErr } = await sb.from("trade_accounts").select("id,cash_balance").eq("id", accountId).maybeSingle();
        if (acctErr) throw new Error(acctErr.message);
        const { data: positions, error: posErr } = await sb.from("trade_positions").select("symbol,qty,avg_entry_price,realized_pnl_usd,updated_at").eq("account_id", accountId);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__59d3ce88._.js.map